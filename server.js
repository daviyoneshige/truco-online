...
io.on('connection', (socket) => {
  ...
  // Registra nome assim que jogador loga (antes de entrar em qualquer fila)
  socket.on('registrar_nome', ({ nome }) => {
    socket.nomeJogador = nome;
    console.log('📝 Registrado:', nome, socket.id);
  });

  // ─── CONVITES ────────────────────────────────────────────
  socket.on('enviar_convite', ({ para, de }) => {
    if (de) socket.nomeJogador = de; // garante nome registrado
    const alvo = [...io.sockets.sockets.values()].find(s => s.nomeJogador === para);
    console.log('📨 Convite de', socket.nomeJogador, 'para', para, '— alvo encontrado:', !!alvo);
  ...
  socket.on('2v2_convidar_parceiro', ({ para, de }) => {
    if (de) socket.nomeJogador = de; // garante nome registrado
    socket.lobbyId = 'lobby_' + Date.now();
    salaLobby2v2.set(socket.lobbyId, { lider: socket, parceiro: null, sockets: [socket] });
    const alvo = [...io.sockets.sockets.values()].find(s => s.nomeJogador === para);
    console.log('🎮 2v2 convite de', socket.nomeJogador, 'para', para, '— alvo:', !!alvo);
