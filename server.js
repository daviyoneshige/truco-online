...
io.on('connection', (socket) => {
  ...
  socket.on('aceitar_truco', () => {
    ...
    if (sala.botTime) enviarEstadoBot(socket.salaId, socket);
    else enviarEstado(socket.salaId);
  });
  ...
  socket.on('correr_truco', () => {
    ...
    if (sala.botTime) {
      enviarEstadoBot(socket.salaId, socket);
      if (!verificarFimJogoBot(socket.salaId, socket))
        setTimeout(() => { novaMao(socket.salaId); enviarEstadoBot(socket.salaId, socket); }, 1500);
    } else {
  ...
  socket.on('resposta_mao_onze', ({ aceita }) => {
    ...
    const enviar = () => isBot ? enviarEstadoBot(socket.salaId, socket) : enviarEstado(socket.salaId);
    const fimFn = () => isBot ? verificarFimJogoBot(socket.salaId, socket) : verificarFimJogo(socket.salaId);
    if (!aceita) {
      sala.pontos[tOnze] += 1;
      sala.log = sala.nomes[tOnze] + ' ganha 1 ponto (adversário recusou).';
      enviar();
      if (!fimFn()) setTimeout(() => { novaMao(socket.salaId); enviar(); }, 1500);
    } else {
