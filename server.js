...
app.post('/api/bot/entrar', async (req, res) => {
  const { conta, nomeJogador } = req.body;
  if (!CONTAS_BOT.includes(conta)) return res.json({ ok: false, msg: 'Conta não autorizada.' });

  // Verifica se o jogador já está na fila
  const socketJogador = filaEspera.find(s => s.nomeJogador === nomeJogador);
  if (!socketJogador) return res.json({ ok: false, msg: 'Entre na fila primeiro.' });

  // Remove o jogador da fila e cria sala com bot
  filaEspera.splice(filaEspera.indexOf(socketJogador), 1);
  const salaId = 'bot_' + Date.now();
  const botNome = conta + '_BOT';
  socketJogador.join(salaId);
  socketJogador.salaId = salaId;
  socketJogador.time = 't1';

  const estado = criarEstadoInicial(nomeJogador, botNome);
  estado.botTime = 't2';
  salas.set(salaId, estado);

  socketJogador.emit('partida_iniciada', { salaId, nomeT1: nomeJogador, nomeT2: botNome });
  enviarEstadoJogador(salaId, socketJogador);

  // Inicia o loop do bot
  setTimeout(() => tickBot(salaId), 1500);
