...
const botsAtivos = new Map(); // salaId -> { time, intervalo }

app.post('/api/bot/entrar', async (req, res) => {
  const { conta } = req.body;
  if (!CONTAS_BOT.includes(conta)) return res.json({ ok: false, msg: 'Conta não autorizada para bot.' });
  // Cria um "socket fake" interno que entra na fila
  const botSocket = {
    id: 'bot_' + Date.now(),
    nomeJogador: conta + '_bot',
    backId: 'padrao',
    salaId: null,
    time: null,
    isBot: true,
    emit: (ev, data) => {
      if (ev === 'partida_iniciada') iniciarJogadaBot(botSocket.salaId, botSocket.time);
      if (ev === 'estado_atualizado') agirComoBot(botSocket.salaId, botSocket.time);
      if (ev === 'adversario_desconectou') pararBot(botSocket.salaId);
    },
    join: (salaId) => { botSocket.salaId = salaId; },
  };

  if (filaEspera.length > 0) {
    const adv = filaEspera.shift();
    const salaId = 'sala_' + Date.now();
    botSocket.join(salaId); adv.join(salaId);
    botSocket.time = 't2'; adv.time = 't1';
    const estado = criarEstadoInicial(adv.nomeJogador, botSocket.nomeJogador);
    estado.backT1 = adv.backId || 'padrao';
    estado.backT2 = 'padrao';
    estado.botTime = 't2';
    salas.set(salaId, estado);
    io.to(salaId).emit('partida_iniciada', { salaId, nomeT1: adv.nomeJogador, nomeT2: botSocket.nomeJogador });
    enviarEstado(salaId);
    iniciarJogadaBot(salaId, 't2');
  } else {
    // Bot fica na fila como entrada fake
    filaEspera.push(botSocket);
    console.log('🤖 Bot ' + conta + ' entrou na fila.');
  }
  res.json({ ok: true });
});

app.post('/api/bot/sair', (req, res) => {
  const { conta } = req.body;
  // Remove bot da fila se estiver esperando
  const idx = filaEspera.findIndex(s => s.nomeJogador === conta + '_bot');
  if (idx !== -1) filaEspera.splice(idx, 1);
  res.json({ ok: true });
});

function iniciarJogadaBot(salaId, botTime) {
  setTimeout(() => agirComoBot(salaId, botTime), 1200);
}

function agirComoBot(salaId, botTime) {
  const sala = salas.get(salaId);
  if (!sala || sala.fim) return;

  // Bot responde truco
  if (sala.aguardandoTruco && sala.quemPediuTruco !== botTime) {
    setTimeout(() => {
      if (!salas.get(salaId)) return;
      // Aceita truco se tiver carta boa, senão corre
      const mao = sala.maos[botTime];
      const temBoa = mao.some(c => forca(c, sala.manilha) > 7);
      if (temBoa) {
        sala.aguardandoTruco = false;
        sala.log = 'Truco aceito! Mão vale ' + sala.valMao + ' pt(s).';
      } else {
        ...
        const SEQ = ['nenhum','truco','seis','nove','doze'];
        const VALS_T = { nenhum:1, truco:3, seis:6, nove:9, doze:12 };
        const valAnt = VALS_T[SEQ[SEQ.indexOf(sala.estadoTruco) - 1]] || 1;
        sala.pontos[sala.quemPediuTruco] += valAnt;
        sala.log = sala.nomes[botTime] + ' correu! ' + sala.nomes[sala.quemPediuTruco] + ' ganha ' + valAnt + ' pt(s).';
      }
      enviarEstado(salaId);
      if (!verificarFimJogo(salaId) && !sala.aguardandoTruco && sala.turno === botTime) {
        setTimeout(() => agirComoBot(salaId, botTime), 1000);
      }
    }, 900);
    return;
  }
  ...
  // Mão de onze
  if (sala.aguardandoMaoOnze && sala.turno !== botTime) {
    setTimeout(() => {
      if (!salas.get(salaId)) return;
      sala.aguardandoMaoOnze = false;
      sala.log = 'Mão de Onze aceita!';
      enviarEstado(salaId);
      if (sala.turno === botTime) setTimeout(() => agirComoBot(salaId, botTime), 1000);
    }, 900);
    return;
  }

  if (sala.turno !== botTime || sala.aguardandoTruco) return;

  setTimeout(() => {
    const sala2 = salas.get(salaId);
    if (!sala2 || sala2.fim || sala2.turno !== botTime || sala2.aguardandoTruco) return;
    const mao = sala2.maos[botTime];
    if (!mao || mao.length === 0) return;

    // Estratégia: joga a carta mais fraca primeiro, guardando as boas
    const idxCarta = mao.reduce((minI, c, i) =>
      forca(c, sala2.manilha) < forca(mao[minI], sala2.manilha) ? i : minI, 0);

    sala2.mesa.push({ time: botTime, carta: mao.splice(idxCarta, 1)[0] });
    sala2.turno = botTime === 't1' ? 't2' : 't1';
    if (sala2.mesa.length === 2) resolverRodada(salaId);
    else enviarEstado(salaId);
  }, 1000);
}

function pararBot(salaId) {
  salas.delete(salaId);
}
