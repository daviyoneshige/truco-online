...
// O t1 recebe suas cartas reais + quantidade das cartas do t2 (e vice-versa).
// Isso garante que cada um vê apenas suas próprias cartas, nunca as do adversário.
// Também resolve o bug das cartas não aparecerem: cada jogador sabe exatamente
// quais são "suas" cartas sem depender de nenhuma variável do front-end.
function enviarEstado(salaId) {
  const sala = salas.get(salaId);
  if (!sala) return;

  // Campos comuns para os dois jogadores
  const base = {
    pontos:            sala.pontos,
    nomes:             sala.nomes,
    mesa:              sala.mesa,
  ...
  // Para cada socket da sala, manda um estado sob medida
  // 'sockets' retorna todos os sockets conectados naquele room
  io.in(salaId).fetchSockets().then(sockets => {
    sockets.forEach(s => {
      const meuTime = s.time;           // 't1' ou 't2'
      const advTime = meuTime === 't1' ? 't2' : 't1';

      s.emit('estado_atualizado', {
        ...base,
        meuTime,                              // front-end sabe qual time é
        minhasCartas: sala.maos[meuTime],     // cartas reais do jogador
        qtdAdversario: sala.maos[advTime].length, // só a quantidade do adversário
      });
    });
  });
}
